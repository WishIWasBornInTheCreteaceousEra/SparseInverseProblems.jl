...
solveFiniteDimProblem(model :: BoxConstrainedDifferentiableModel, loss :: LSLoss, thetas :: Array{Float64, 2}, y :: Array{Float64,1}, tau :: Float64) = nnlasso(hcat([psi(model, vec(thetas[:,i])) for i = 1:size(thetas,2)]...),y,tau)
#Changed thetas data type and y data type
