...
function getStartingPoint(model :: GaussBlur2D, v :: Vector{Float64})
  ...
  best_point_idx = i2s(zeros(ng,ng), best_point_lin_idx)
...
#Changed data type of thetas and y
function solveFiniteDimProblem(model :: GaussBlur2D, loss :: LSLoss, thetas :: Array{Float64, 2}, y :: Array{Float64,1}, tau :: Float64)
  ...
  K = K_x.*K_y
  AtY = vec(sum((t_vecs[3]'*y).*(t_vecs[1]'),dims=2))
