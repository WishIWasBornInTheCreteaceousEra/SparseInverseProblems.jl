include("../../src/SparseInverseProblems.jl")
include("gaussblur.jl")
using Printf, FileIO, Images
...
function runFW(sim,imageArray; n_cd_iters :: Int64 = 200)
  ...
  results = Array{Array{Float64,2},1}(undef, nImages)
...
println("The time for reading the images was,")
@time imageArray = readImages("data/sequence",nImages,64)


print("Running FWA-MI")
@time begin
results = runFW(gb_sim,imageArray)
println(" done. The wall time on the optimization was, ")
end
